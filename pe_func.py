...
def get_cps_data(start_dt, end_dt, corp_nm):
    ...
    if df.empty == False :
        ...
        df['전환청구종료일'] = df['전환청구종료일'].str.replace(pat=r'[ㄱ-ㅣ가-힣]+', repl=r'', regex=True).str.replace(' ', '').str.replace('.', '')
        df = df.rename(columns={'전환조건':'전환가액 조정에 관한 사항'})
        df = df[['발행사', '공시일', '신주의 종류와 수', '1주당 액면가액', '증자방식', '전환비율', '전환가액결정방법', '전환주식종류', '전환주식수', '주식총수대비비율', '전환청구시작일', '전환청구종료일', '전환가액 조정에 관한 사항','의결권', '신주발행가액', '할인율 또는 할증율(%)']]
    return df
